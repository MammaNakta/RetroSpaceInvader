...
### 2. 프로젝트 초기 셋업 (1-Click Setup)
새로 클론한 환경에서 프리팹, 스프라이트(Point 필터/1 PPU), 씬 설정을 1초 만에 자동 완성할 수 있습니다:
1. 유니티 에디터 상단 메뉴 바에서 **`Retro Space Invader` > `Build & Setup Project`** 를 클릭합니다.
2. `Assets/Scenes/MainGameScene.unity` 씬이 생성/저장되며 Build Settings에 자동 등록됩니다.
3. 해당 씬을 열고 상단 **`Play (▶)`** 버튼을 누르면 즉시 테스트가 가능합니다.
...
- [ ] 스페이스바를 연타해도 **화면 내에 동시에 존재하는 미사일은 최대 2발**로 제한되는지 확인.
